...
- `experience_reward`
- `is_aggro`
- `is_named` — explicit boolean for article-free named NPC rendering; set `true` only for unique named/story NPCs
- `is_ally`
- `is_peaceful`
- `respawn`
...
- `is_companion: true` marks a template as an enlistable AI companion. Companion templates must set `respawn: false` and `is_ally: true`, must never appear in any room's `npcs` list, and cannot also set `is_merchant` or `is_recruiter`. Give companions explicit `max_vigor`/`max_mana` pools or their skills and spells never fire.
- Companion stats scale with the owning player's level at spawn and each combat round: the template values are the level 1 baseline, and hit points, resource pools, power_level, and hit roll modifiers grow per owner level (rates configured in the server settings `gameplay` section).
- `is_guardian: true` (companions only) marks a tank companion: while its owner, an owner's group member, or one of their companions is actively targeted by an enemy in the room, it taunts instead of fighting. Every such enemy rolls separately against the taunt (chance set by the server `companion_taunt_chance` setting); pulled enemies attack the guardian indefinitely, until it dies, is rescued, or leaves the room. Enemies already held by another guardian are left alone.
- `voice_lines` - optional object mapping categories to lists of full third-person personality lines. Supported categories: `enlist` (spoken when hired), `dismiss` (spoken when released), `victory` (occasional quip when the fight's target falls, chance set by the server `companion_voice_chance` setting). One line is picked at random per event.
