...
- `is_guardian: true` (companions only) marks a tank companion: while any ally in the room is actively targeted by an enemy, it taunts instead of fighting. Every such enemy rolls separately against the taunt (chance set by the server `companion_taunt_chance` setting); pulled enemies attack the guardian indefinitely, until it dies, is rescued, or leaves the room.
